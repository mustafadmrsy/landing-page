document.addEventListener('DOMContentLoaded', function() {
    ...
    // Global blogData değişkenini kontrol et ve kullan
    // Eğer yoksa, yerel veri yapısını kullan
    let blogPostsData;
    
    if (window.blogData && window.blogData.blogPosts) {
        console.log('Global blogData kullanılıyor');
        // Global veriyi kategorilere göre düzenle
        blogPostsData = {};
        ...
        document.querySelectorAll('.category-item').forEach(item => {
            ...
            if (categoryName) {
                blogPostsData[categoryName] = [];
            }
        });
        
        // Blog yazılarını kategorilere göre filtrele
        window.blogData.blogPosts.forEach(post => {
            if (post.tags && post.tags.length > 0) {
                post.tags.forEach(tag => {
                    if (blogPostsData[tag]) {
                        blogPostsData[tag].push({
                            title: post.title,
                            author: post.author,
                            date: post.date,
                            preview: post.summary || post.content.substring(0, 150) + '...',
        ...
        console.log('Yerel blog verileri kullanılıyor');
    ...
    function displayBlogPosts(category) {
        ...
        const posts = blogPostsData[category] || [];
